...
def plot_heatmap(data, figsize=None):
    ...
    return sns.heatmap(data, annot=True, cmap='Reds')
...
def apply_pca(cnts, n_components=14):
    ...
    return aligned_concat(pd.DataFrame(pca.components_, columns=cnts.columns).rename_axis(['component']), pd.Series(pca.explained_variance_ratio_, name='explained_variance'))



def get_prices_with_building_cnts(world, property_type):
    building_counts = world.groupby(['outcode', 'building']).price.count().unstack('building').fillna(0)
    ...
    median_prices = world.query(f"property_type == '{property_type}'").groupby('outcode').price.median().to_frame()
    return median_prices.join(building_counts, how='left')

def get_prices_with_amenity_cnts(world, property_type):
    amenity_counts = world.groupby(['outcode', 'amenity']).price.count().unstack('amenity').fillna(0)
...
def get_price_and_plot_area_correlation(data, property_types):
    ...
    corrs_list = []
    for _, data in data.query("property_type.isin(@property_types)").assign(area_code=lambda df: df.postcode.str[:-2]).groupby('area_code')[['price', 'plot_area']]:
        if len(data) < 5:
            continue
...
def plot_polygons_on_map(df, init_lat=None, init_lon=None, zoom_start=10):
    ...
    m = folium.Map(location=[init_lat, init_lon], zoom_start=zoom_start, tiles="CartoDB positron")
...
def get_amenities_per_district(place, db, threshold=5):
    districts = get_districts(place, db)
    amenities = load_place_tags(place, {"amenity": True})
    amenities_per_district = districts.sjoin(amenities, how='left', predicate='contains')
    amenity_cnt = amenities_per_district.groupby(['outcode', 'amenity']).geometry.count().unstack('amenity').fillna(0).pipe(lambda df: df.loc[:, df.mean() > threshold])
    return amenity_cnt
...
def get_k_means_metrics_raw(world, n_clusters=100):
    ...
    for i in range(4):
        ...
        print(
            f"Fleiss Kappa for groups by the last {i} digits of post code: {(good_pairs - chance_pairs) / (all_pairs - chance_pairs)}"
        )
...
def get_k_means_metrics_inflation_adjusted(world, n_clusters=100):
    ...
    dfx = world.merge(quarterly_medians, how="left", left_on="quarter", right_index=True)
    dfx = dfx.assign(
...
def count_freeholds_and_leaseholds(world):
    return world.reset_index().groupby(["property_type", "tenure_type"]).db_id.count().unstack().rename(columns={'F': "Freehold", 'L': "Leasehold"})
...
def plot_other_infos(world):
    ...
    axis.set_title("Distribution with and without 'other' (blue and orange respectively)")

    axis = next(ax)
    axis.set(xscale="log")
    sns.histplot(world.pipe(lambda data: data[data.property_type == "O"]).price, bins=300, ax=axis)
    axis.set_title("Price distribution of 'other'")
    return plt.tight_layout(pad=5)
        


def get_districts(place, db):
    def fetch_districts(world):
        districts_list = []
        outcode_list = []
        for outcode, grp in tqdm(world.to_crs(crs=3857).groupby("outcode")):
            district = grp.buffer(1000).unary_union
            districts_list.append(district)
            outcode_list.append(outcode)
    
        districts = gpd.GeoDataFrame(outcode_list, columns=['outcode'], geometry=districts_list, crs=3857).to_crs(4326)
        return districts
...
def plot_log_prices_per_property_type(world):
    ...
    for property_type in ["O", "D", "S", "T", "F"]:
        ...
        axis = next(ax)
        sns.histplot(dfx.price.pipe(np.log), ax=axis).set_title(f"Price distribution | property_type = {property_type}")
...
def plot_log_prices_per_sq_m_per_property_type(world):
    ...
    for property_type in ["D", "S", "T"]:
        dfx = world.query("property_type == @property_type")
        axis = next(ax)
        sns.histplot(dfx.price.div(dfx.area).pipe(np.log), ax=axis).set_title(f"Price per sq. m | property_type = {property_type}")
        axis.set(xlabel="log_price")
...
def get_house_prices_per_town(db, min_count=300):
    ...
    data = get_or_load(
        "town_median_price", fetch_house_prices_per_town, db=db, min_count=min_count, cache_dir="../cache/sql/"
    )
...
def plot_choropleth(towns, price_per_town, init_lat=None, init_lon=None):
    ...
    m = folium.Map(location=[init_lat, init_lon], zoom_start=7, tiles="CartoDB positron")
