...
def load_place_prices(place, db, force_reload=False):
    ...
    ).drop_duplicates(subset=['db_id']).reset_index().set_index('db_id')
...
def plot_tx_count_over_time(db):
    ...
    axis = next(ax)
    dates.set_index("date_of_transfer").rolling(120).mean().plot(ax=axis, title='Transaction Count Over Time')
    
    axis = next(ax)
    dates.set_index("date_of_transfer").pipe(lambda df: df.rolling(30).mean().sub(df.rolling(120).mean())).rename(columns={'transaction count': 'tx_count change'}).plot(ax=axis, title="Transaction Count Change In Mean")
    for i in range(2018, 2023):
        axis.axvspan(pd.Timestamp(f"{i}-06-01"), pd.Timestamp(f"{i}-09-01"), color='green', alpha=0.5)
...
def plot_outliers(db, force_reload=False):
    ...
    outlier_prices.query("dir == 'HIGH'").price.plot(logy=True, ax=next(ax), xlabel='n-th highest price', ylabel='price', title='10000 highest prices')
    outlier_prices.query("dir == 'LOW'").price.plot(logy=True, ax=next(ax), xlabel='n-th highest price', ylabel='price', title='10000 lowest prices')
...
def plot_log_prices_around(gdf, world, radius=1000):
    """Radius is in meters"""
    axis = subplots_iter(math.floor(math.sqrt(len(gdf))), max_subplots=len(gdf), subplot_height=10, subplot_width=10)
    for idx, data in gdf.iterrows():
        ...
        base = buildings.plot(ax=ax, color="white", edgecolor="black", alpha=0.4, aspect='equal')
        buildings.assign(ax=ax, log_price=lambda df: df.price.pipe(np.log10)).plot(
            ax=base, column="log_price", edgecolor="black", legend=True,
